...
if __name__ == '__main__':
    ...
    for mall_id in malls:
        ...
        if not op.exists(save_dir):
            ...
            print(get_time(), ' saved a model for ', mall_id, ' score: ', score)
            train_time = int(train_time)
            sql = "UPDATE scores SET xgb='{s}',xgb_itr_times={t}, xgb_train_time={tt} WHERE mall_id='{m}'".format(s=score, t=n_est, m=mall_id,tt=train_time)
            cur.execute(sql)
            conn.commit()
